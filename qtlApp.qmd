...
```{r}
peaks <- suppressMessages(qtlApp::peak_finder(import$file_directory, selected_dataset))
```
...
selected_peak <- subset(peaks, trait == selected_trait)
```
...
```{r}
peaks <- suppressMessages(qtlApp::peak_finder(import$file_directory, selected_dataset,
  qtlApp::get_selected_trait(import, which_trait, selected_dataset)))
```
...
```{r}
selected_peak <- subset(peaks, marker == which_peak)
```
...
```{r}
qtl_obj <- qtlApp::QTL_plot_visualizer(
  scans, which_trait, LOD_thr, import$markers)
```

```{r}
scan_plot <- qtlApp::ggplot_qtl_scan(qtl_obj)
ggplot2::ggsave("scan.pdf", scan_plot)
```

```{r}
qtl_peak <- dplyr::filter(qtl_obj, LOD == max(LOD))
...
qtlApp::ggplotly_qtl_scan(scan_plot, selected_chr)
```
...
- `selected_dataset` initial selected not showing
