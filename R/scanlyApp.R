    ...
    shiny::observeEvent(click_data(), {
      if (!is.null(click_data)) {
        # Process the click data
          ...
          DT::renderDT({
            # Create a data frame with the clicked point information
            # ** see point_info and datatable in Kalynn's app **
            clicked_point_info <- data.frame(
              x = selected_x,
              y = selected_y
            )
            # Display the clicked point information in a table
            DT::datatable(clicked_point_info, options = list(dom = 't', paging = FALSE))
          })
      }
    })
