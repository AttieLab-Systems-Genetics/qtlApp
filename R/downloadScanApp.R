#' Download Scan App Module
#'
#' This is an adaptation from Kalynn code to download the QTL plot.
#' Plan is to use downloaddApp from foundrShiny package instead.
#' 
...
downloadScanApp <- function() {
  ui <- bslib::page_sidebar(
    ...
    sidebar = bslib::sidebar("side_panel",
      ...
      downloadScanInput("download")), # "download"
    bslib::card(
  ...
  server <- function(input, output, session) {
    ...
    downloadScanServer("download", main_par, peak_table, scan_table)
  }
  shiny::shinyApp(ui = ui, server = server)
}
#' @rdname downloadScanApp
#' @export
downloadScanServer <- function(id, main_par, peak_table, scan_table, plot_result) {
  shiny::moduleServer(id, function(input, output, session) {
    ns <- session$ns
    output$downloadScanInput <- shiny::renderUI({
      list(
...
#' @rdname downloadScanApp
#' @export
downloadScanInput <- function(id, main_par, import) {
  ns <- shiny::NS(id)
  shiny::uiOutput(ns("downloadScanInput"))
}
