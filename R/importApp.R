...
importServer <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    create_cache() # This function currently does nothing, might be for future use
    
   
    base_data_dir <- Sys.getenv("SHINY_DATA_DIR", unset = "/data/dev/miniViewer_3.0") # Use env var or default
    ...
    if (!is.list(loaded_data_list)) {
        ...
        loaded_data_list <- list(file_directory = loaded_data_list) # Example if it just returned file_dir
    }
